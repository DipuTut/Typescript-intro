...
function bestimmeJahreszeit(monat: string): string {
...
function istPasswortGueltig(passwort: string): string {
...
function multiplikationstabelle(n: number): number {
 for ( var i = 1; i <=10; i++) {
  let m = n * i;
 console.log(m); 
}
...
function filterGeradeZahlen(arr: string) {
  let geradeZahlen = [];
